...
import TutorDataToServerTwoEdit from './tutorDataToServerTwoEdit';
...
const FormTutorEdit = () =>{
    ...
    // Tutor registered
    const [tutorRegistered, setTutorRegistered] = useState([]);
    ...
    const registeredTutor = async () => {
          if(tutors.length > 0){
            ...
            if( tutorWithSameEmail.length > 0 ){
                setTutorRegistered(tutorWithSameEmail);
                dispatch(modifier(["price", tutorWithSameEmail[0].price]));
                dispatch(modifier(["description", tutorWithSameEmail[0].description]));
                dispatch(modifier(["image", tutorWithSameEmail[0].image]));
            }
          }
      }
    
    // Los datos del tutor están registrados? 
    if(tutorRegistered < 1){
        registeredTutor();
    }
    ...
    return(
        ...
        <div className="box__formTutorIni panel is-info">
            ...
            <h4 className="panel-heading title is-4 TitleFormTutorIni"> Edite sus datos como monitor académico </h4>
            ...
            <div className="field">
                ...
                <div className="control">
                    ...
                    onChange={handleChange}
                    maxLength="6"
                    defaultValue={ tutorRegistered.length > 0 ? tutorRegistered[0].price : ""}
                    />
                </div>
                {
            ...
            <div className="field">
                ...
                rows="10"
                onChange={handleChange}
                defaultValue={ tutorRegistered.length > 0 ? tutorRegistered[0].description : ""}
                ></textarea>
                {
            ...
            {
                tutorRegistered.length > 0
                ?
                <TutorDataToServerTwoEdit buttomName="Editar" _id={tutorRegistered[0]._id}/>
                :
                <></>
            }
        </div>
